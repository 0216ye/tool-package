...
## 8. 数组取差异与合并
    ...
    2. mergeArray(arr1, arr2): 
        ...
        如: mergeArray([1,3,5,7], [5, 8]) ==> [1, 3, 5, 7, 8]
...
## 15 手写继承
    实现方式:  
        寄生组合式
            1.子类构造函数中使用call借用父类构造函数，得到与父类相同的属性
            2.子类指定新的原型对象为父类，得到父类原型上的方法
            3.寄生处理：
                去除原型对象上多余的属性-->不直接new 父类构造函数
                    通过指定一个函数的原型对象为父类的原型对象,然后直接new该函数
    解决问题: 
        原型对象有不必要的属性
        多调用了1次父类型构造函数
